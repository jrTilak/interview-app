...
export class InterviewAttemptsService {
	...
	/** Creates the candidate's single attempt for a share link or resumes it. */
	async createOrResume(
		...
		const attemptId = await this._database.transaction(async (transaction) => {
			...
			const [created] = await transaction
				...
				.onConflictDoNothing({
					target: [interviewAttempt.interviewId, interviewAttempt.candidateId],
				})
			...
			const [existing] = await transaction
				...
				.limit(1);
			if (!existing)
				throw new ConflictException("Attempt could not be resumed");
