/**
 * Generated by orval v8.23.0 🍺
 * Do not edit manually.
 * Interview Desk API
 * Interview creation and realtime candidate-attempt API.
 *
 * Application routes require a Better Auth session cookie unless explicitly documented as public. Request bodies and parameters are strictly validated. Validation failures use HTTP 422, ownership checks hide foreign resources as 404, and unexpected server failures are sanitized.
 *
 * New interviews are private. Their owner publishes or unpublishes them through `PATCH /api/interviews/:id` with `isPublic`; public previews and authenticated candidate attempts use the interview UUID itself rather than a separate share code.
 *
 * OpenAPI spec version: 0.1.0
 */
import type { InterviewDetailsResponseDtoQuestionsItem } from './interviewDetailsResponseDtoQuestionsItem';

export interface InterviewDetailsResponseDto {
  /**
     * Interview UUID and public share identifier.
     * @pattern ^([0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-8][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}|00000000-0000-0000-0000-000000000000|ffffffff-ffff-ffff-ffff-ffffffffffff)$
     */
  id: string;
  /**
     * Interview title.
     * @minLength 3
     * @maxLength 160
     */
  title: string;
  /**
     * Optional interview context.
     * @minLength 1
     * @maxLength 2000
     * @nullable
     */
  description: string | null;
  /** Interview time limit in minutes. */
  durationMinutes: number;
  /** Whether a candidate may attempt the interview more than once. */
  allowMultipleAttempts: boolean;
  /** Number of structured questions in the interview. */
  questionCount: number;
  /** Whether candidates can access the interview through its ID. */
  isPublic: boolean;
  /**
     * Date and time when the interview was created.
     * @pattern ^(?:(?:\d\d[2468][048]|\d\d[13579][26]|\d\d0[48]|[02468][048]00|[13579][26]00)-02-29|\d{4}-(?:(?:0[13578]|1[02])-(?:0[1-9]|[12]\d|3[01])|(?:0[469]|11)-(?:0[1-9]|[12]\d|30)|(?:02)-(?:0[1-9]|1\d|2[0-8])))T(?:(?:[01]\d|2[0-3]):[0-5]\d(?::[0-5]\d(?:\.\d+)?)?(?:Z|([+-](?:[01]\d|2[0-3]):[0-5]\d)))$
     */
  createdAt: string;
  /**
     * Private topic notes supplied by the interview creator.
     * @minLength 1
     * @maxLength 2000
     */
  rawQuestions: string;
  /** Ordered questions prepared for the interview. */
  questions: InterviewDetailsResponseDtoQuestionsItem[];
}
