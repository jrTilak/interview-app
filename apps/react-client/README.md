...
The React client is a desktop-only PWA with separate Interview and Recruiter workspaces. It uses Vite, Chakra UI, TanStack Router/Query/Form, Zustand, Axios, Socket.IO, MediaPipe, Orval, and Vitest.
...
- Camera and monitor tracks are encoded with `MediaRecorder` only when the corresponding global development flag is enabled. The server validates and discards these disposable chunks.
- Screen acquisition requests and validates an entire monitor when whole-screen enforcement is enabled; a browser tab or application window is rejected.
- MediaPipe face detection uses a bundled model in the browser. The preview draws outlines, one face can be required to start, and zero/multiple faces can pause or terminate based on global flags.
- The microphone is converted in the browser to mono 16 kHz signed little-endian PCM16 for the socket protocol; the server sends each completed turn to local STT.
...
In development, `/__flags__` changes the server-wide integrity and media behaviors. The client refreshes the snapshot so changes affect active users without a reload.
